...
TO_NAPI_IMPL(webrtc::RTCError *, pair) {
  return Converter<std::pair<Napi::Env, const webrtc::RTCError *>,
                   Napi::Value>::Convert(pair);
}
...
TO_NAPI_IMPL(const webrtc::RTCError *, pair) {
  ...
  case webrtc::RTCErrorType::INVALID_RANGE:
    return Pure(
        scope.Escape(ErrorFactory::CreateRangeError(env, error->message())));
  case webrtc::RTCErrorType::SYNTAX_ERROR:
    return Pure(
        scope.Escape(ErrorFactory::CreateSyntaxError(env, error->message())));
  case webrtc::RTCErrorType::INVALID_STATE:
    return Pure(scope.Escape(
        ErrorFactory::CreateInvalidStateError(env, error->message())));
  case webrtc::RTCErrorType::INVALID_MODIFICATION:
    return Pure(scope.Escape(
        ErrorFactory::CreateInvalidModificationError(env, error->message())));
  case webrtc::RTCErrorType::NETWORK_ERROR:
    return Pure(
        scope.Escape(ErrorFactory::CreateNetworkError(env, error->message())));
  ...
  case webrtc::RTCErrorType::OPERATION_ERROR_WITH_DATA:
    return Pure(scope.Escape(
        ErrorFactory::CreateOperationError(env, error->message())));
  }
}
