...
static Validation<UNSIGNED_SHORT_RANGE>
UNSIGNED_SHORT_RANGE_FN(const Maybe<uint16_t> &maybeMin,
                        const Maybe<uint16_t> &maybeMax) {
  auto min = maybeMin.FromMaybe(0);
  auto max = maybeMax.FromMaybe(65535);
  if (min > max) {
    return Validation<UNSIGNED_SHORT_RANGE>::Invalid(
...
TO_NAPI_IMPL(UNSIGNED_SHORT_RANGE, pair) {
  ...
  if (value.min.IsJust()) {
    NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "min",
                                          value.min.UnsafeFromJust())
  }
  if (value.max.IsJust()) {
    NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "max",
                                          value.max.UnsafeFromJust())
  }
