...
CONVERTER_IMPL(const webrtc::RTCError *, SomeError, error) {
  if (!error) {
    return Validation<SomeError>::Invalid(
        "No error? Please file a bug on "
        "https://github.com/node-webrtc/node-webrtc");
  }
  auto type = MakeRight<ErrorFactory::DOMExceptionName>(
      ErrorFactory::ErrorName::kError);
  switch (error->type()) {
  ...
  case webrtc::RTCErrorType::INVALID_PARAMETER:
    type = MakeLeft<ErrorFactory::ErrorName>(
        ErrorFactory::DOMExceptionName::kInvalidAccessError);
    break;
  case webrtc::RTCErrorType::INVALID_RANGE:
    type = MakeRight<ErrorFactory::DOMExceptionName>(
        ErrorFactory::ErrorName::kRangeError);
    break;
  ...
  case webrtc::RTCErrorType::NETWORK_ERROR:
    type = MakeLeft<ErrorFactory::ErrorName>(
        ErrorFactory::DOMExceptionName::kNetworkError);
    break;
  case webrtc::RTCErrorType::INVALID_STATE:
  // NOTE(mroberts): SetLocalDescription in the wrong state can throw this.
  case webrtc::RTCErrorType::INTERNAL_ERROR:
    type = MakeLeft<ErrorFactory::ErrorName>(
        ErrorFactory::DOMExceptionName::kInvalidStateError);
    break;
  ...
  case webrtc::RTCErrorType::OPERATION_ERROR_WITH_DATA:
    type = MakeLeft<ErrorFactory::ErrorName>(
        ErrorFactory::DOMExceptionName::kOperationError);
    break;
  }
...
TO_NAPI_IMPL(SomeError, pair) {
  ...
  return Pure(scope.Escape(someError.name().FromEither<Napi::Value>(
